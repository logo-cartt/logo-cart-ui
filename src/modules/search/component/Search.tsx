...
export function Search() {
  ...
  const handleFetchData = () => {
    ...
    const filteredProducts = response.filter((product) => product.name.toLowerCase().includes(queryProduct))
      ...
      <Grid item={true} xs={12} textAlign="left" pt={4} pb={4}>
        ...
        <Grid container={true} spacing={5}>
          {filteredProducts.map((filteredProduct) => {
            return (
              <Grid item={true} xs={12} sm={6} md={4} lg={3} key={filteredProduct.id}>
                <ProductCard
